...
export default function Chat() {
    ...
    //     messagesRemaning--
    //     const newMessage = { sender: 'you', content: input, messageId }
    ...
    //                 const newResponse = { sender: 'vegabot', content: response, messageId }
    ...
    //     const requestBody = { input: input, userId: userId, messageId }
    ...
    //             const newResponse = { sender: 'vegabot', content: newContent, messageId}
    //             setMessages([...messages, newMessage, newResponse])
    //         } else {
    //             messagesRemaning--
    ...
    //             const newResponse = { sender: 'vegabot', content: newContent, sources: source, messageId}
    //             setMessages([...messages, newMessage, newResponse])
    //         }
    ...
    useEffect(() => {
        ...
        messages.map((message) => (
            message.result ? setIsLoading(false) : setIsLoading(true)
        ))
    }, [messages])
                            ...
                            <CardContent className='flex flex-col items-center'>
                                <ScrollArea className="h-[75vh] w-full px-[2vw] max-lg:h-[93vh] max-lg:w-screen max-lg:px-[10vw]" >
                                    ...
                                    {messages.map((message, index) => (
                                        <div key={index}>
                                            {message.sender === 'you' || message.result && (
                                            ...
                                            {message.result && (
                                                <div key={index} className='flex gap-[2vh] mt-[2vh] mb-5'>
                                                    ...
                                                    <Card className="lg:max-w-[85%] bg-transparent text-[#333333] rounded-[3vh] shadow-inner hover:border-black hover:ease-in-out">
                                                        <CardContent className="flex flex-wrap min-h-[5vh] max-w-[100%] lg:px-[1vw] lg:py-[1.4vh] text-[1.5vh] max-lg:text-sm p-[20px]">
                                                            <MarkdownDisplay content={message.result} />
                                                        </CardContent>
                                                    </Card>
                                                </div>
                                            )}
                                ...
                                <CardFooter className='bottom-[0vh] absolute'>
                                        {isLoading ? (
                                            <div className="flex gap-[1vh]" >
                                                <Input type='text' className='bg-neutral-200 pl-[1.5vh] text-[1.5vh] text-neutral-800 rounded-[1vh] w-[35vw] h-[5vh] max-lg:h-[40px] placeholder:text-neutral-600 placeholder:text-[1.5vh] max-lg:text-sm max-lg:w-[80vw]' placeholder="Aguarde um momento enquanto eu processo sua resposta" value={input} onChange={e => setInput(e.target.value)} />
                                                <Button 
                                                type="submit" 
                                            ...
                                            <div className="flex gap-[1vh]" >
                                                <Input type='text' className='bg-neutral-200 pl-[1.5vh] text-[1.5vh] focus:ring-1 ring-neutral-900 text-black rounded-[1vh] w-[35vw] h-[5vh] max-lg:h-[40px] placeholder:text-neutral-600 placeholder:text-[1.5vh] max-lg:text-sm max-lg:w-[80vw]' placeholder="Me faça uma pergunta sobre o Real Digital" value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyPress} />
                                                <Button 
                                                type="submit" 
                                                className="bg-neutral-200 rounded-[1vh] lg:h-[5vh] lg:w-[3vw] hover:bg-neutral-400" 
                                                onClick={handleInput}> 
